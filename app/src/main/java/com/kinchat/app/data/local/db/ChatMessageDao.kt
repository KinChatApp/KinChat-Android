...
interface ChatMessageDao {
    @Transaction
    @Query("SELECT * FROM messages WHERE chatId = :chatId AND id NOT LIKE 'msg_%_last' AND (isDeletedForMe = 0 OR isDeletedForMe IS NULL) ORDER BY createdAt ASC")
    ...
    suspend fun upsertMessageMerged(serverMessage: ChatMessageEntity) {
        ...
        // 🚀 FIX: Automatically link REAL messages to the Dashboard instantly.
        // Even if a dummy message existed, this forces the dashboard to look at the real message.
        if (!serverMessage.id.endsWith("_last")) {
            updateChatLastMessagePointer(serverMessage.chatId, serverMessage.id, serverMessage.createdAt)
        }
    }

    // Update the chats table pointer if this real message is newer, or if we are currently stuck on a dummy message.
    @Query("""
        UPDATE chats 
        SET lastMessageId = :messageId, lastMessageTime = :time 
        WHERE id = :chatId 
        AND (lastMessageTime IS NULL OR :time >= lastMessageTime OR lastMessageId LIKE 'msg_%_last')
    """)
    suspend fun updateChatLastMessagePointer(chatId: String, messageId: String, time: Long)
    ...
    @Query("""
        ...
        AND id NOT LIKE 'msg_%_last'
    """)
    suspend fun getPendingMessageIds(chatId: String, currentUserId: String): List<String>
    ...
    @Query("""
        ...
        LIMIT 1
    """)
    suspend fun getLatestRealMessage(chatId: String): ChatMessageEntity?
}
